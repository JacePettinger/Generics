...
public class Stack<T> implements PushPop<T> {
    ...
    public void swap(int firstIndex, int secondIndex) {
        if (firstIndex < 0 || secondIndex < 0) {
            System.out.println("Nothing Swapped. Indexes cannot be negative.");
        } else {
            T temp = stack[firstIndex];
            stack[firstIndex] = stack[secondIndex];
            stack[secondIndex] = temp;
            System.out.println("Swapped values at index " + firstIndex + " and " + secondIndex);
        }
    }
